...
| 优先级 | 问题编号 | 描述 | 影响面 |
| --- | --- | --- | --- |
| P0 | 3.2 | `GET /agent-config` 返回不完整 | 技能包配置失效 |
| P0 | 3.3 | 内置 Agent 无法保存技能包 | 功能不可用 |
| P0 | 3.4 | `active` 默认值导致 Agent 不可调度 | 新建 Agent 可能无法使用 |
| P1 | 5.3 | 保存按钮和技能保存混淆 | 用户体验 |
| P1 | 6.1 | 配置更新后注册表不刷新 | 展示不同步 |
| P1 | 3.1 | 两套创建路径不统一 | 维护成本 |
| P1 | 8.2 | slug 唯一性缺少校验 | 数据安全 |
| ~~P2~~ | ~~5.2~~ | ~~配置面板缺少房间设置~~ | ✅ 已调整：改为随机坐标生成 |
| P2 | 4.1 | 中文名称无法生成 slug | 体验优化 |
| P2 | 4.2 | 创建时不设提示词 | 引导不足 |
| P2 | 5.1 | modelDisplay 硬编码 | 展示误导 |
| P1 | 7.3 | 内置 Agent 不可删除，限制过严 | 用户无法清理不需要的内置 Agent |
| P2 | 7.1 | 删除无级联清理 | 数据残留 |
...
当前 Agent 管理功能的核心 CRUD 流程已可用，但在**数据一致性、配置完整性、交互引导**三方面存在较多问题。建议按优先级从 P0 开始逐步修复，重点解决技能包配置失效、`active` 默认值、API 返回不完整等阻塞性问题。
