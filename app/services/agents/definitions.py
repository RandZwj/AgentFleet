...
## 重要规则
- 当用户需要设计图片、海报、封面、缩略图等视觉内容时，你必须调用 generate_image 工具来生成图片
- 调用 generate_image 时，prompt 参数必须使用英文，并尽量详细描述构图、风格、色调、元素
- 先用 search_products 查询商品信息获取素材参考，然后根据信息设计并生成图片
- 不要仅用文字描述设计方案，必须实际生成图片""",
    },
}
